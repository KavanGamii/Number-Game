...
function registerGameHandlers(io, socket, roomManager) {
    ...
    socket.on('game:submit_secret', (payload, callback) => {
        const room = roomManager.getRoomBySocketId(socket.id);
        if (!room)
            return callback({ success: false, error: 'Room not found.' });
        const { room: updatedRoom, error } = roomManager.submitSecretNumber(room.id, socket.id, payload.number, (uRoom) => {
    ...
    socket.on('game:submit_guess', (payload, callback) => {
        ...
        if (!room)
            return callback({ success: false, error: 'Room not found.' });
        const { room: updatedRoom, error } = roomManager.submitGuess(room.id, socket.id, payload.guess, (uRoom) => {
            io.to(uRoom.id).emit('room:updated', uRoom);
        });
        if (error) {
            callback({ success: false, error });
    ...
    const handlePlayerLeave = (socketId, roomId) => {
        const kickedIds = roomManager.leaveRoom(socketId, (updatedRoom) => {
