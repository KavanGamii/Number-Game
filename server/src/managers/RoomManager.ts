import { Room, Player, RoomSettings, ChatMessage, GameState } from '../types/game';

export class RoomManager {
  private rooms: Map<string, Room> = new Map();
  private socketToRoom: Map<string, string> = new Map(); // socket.id -> roomId
  private timers: Map<string, NodeJS.Timeout> = new Map(); // roomId -> Timer
  private roomExpiryTimers: Map<string, NodeJS.Timeout> = new Map();
  public onRoomExpired?: (roomId: string, hostId: string) => void;
  ...
  public isUsernameGloballyUnique(username: string): boolean {
  ...
  public createRoom(settings: Partial<RoomSettings>, isPrivate: boolean, password?: string): Room {
  ...
  public joinRoom(
    ...
    if (room.isPrivate && room.password !== password) {
      return { room: {} as Room, error: 'Invalid room password.' };
    }

    if (room.players.length >= room.settings.maxPlayers) {
      return { room: {} as Room, error: 'Room is full.' };
    }

    if (!this.isUsernameGloballyUnique(username)) {
      return { room: {} as Room, error: 'Username is already taken by another active player.' };
    }
  ...
  public updateSettings(roomId: string, settings: Partial<RoomSettings>): Room | undefined {
  ...
  public toggleReady(roomId: string, playerId: string): Room | undefined {
    const room = this.getRoom(roomId);
  ...
  public kickPlayer(roomId: string, hostId: string, kickPlayerId: string): Room | undefined {
    const room = this.getRoom(roomId);
    if (!room || room.status !== 'lobby') return undefined;

    const host = room.players.find((p) => p.id === hostId);
    if (!host || !host.isHost) return undefined;
  ...
  public startSelectionPhase(roomId: string, onUpdate: (room: Room) => void): Room | undefined {
    const room = this.getRoom(roomId);
    if (!room || (room.status !== 'lobby' && room.status !== 'ended')) return undefined;
    ...
    if (room.status === 'lobby') {
      ...
      if (!allOtherReady) return undefined;
    }
  ...
  private startSelectionTimer(room: Room, onUpdate: (room: Room) => void) {
  ...
  private startGuessingPhase(room: Room, onUpdate: (room: Room) => void) {
    room.status = 'playing';
  ...
  private startTurnTimer(room: Room, onUpdate: (room: Room) => void) {
    this.clearTimer(room.id);
  ...
  public useInterrogation(
    roomId: string,
    playerId: string,
    onUpdate: (room: Room) => void
  ): { room?: Room; error?: string } {
    const room = this.getRoom(roomId);
    if (!room || room.status !== 'playing') return { error: 'Game is not in guessing phase.' };
    if (room.gameState.currentTurnPlayerId !== playerId) return { error: 'It is not your turn.' };

    const player = room.players.find((p) => p.id === playerId);
    if (!player) return { error: 'Player not found.' };
    if (!player.powerUps?.interrogation) return { error: 'You have already used Interrogation this round.' };

    player.powerUps.interrogation = false;
    player.missedTurns = 0;
    this.clearTimer(room.id);

    // Reveal Odd/Even for all active opponents
    const opponents = room.players.filter(p => p.id !== playerId && !p.isEliminated && p.secretNumber !== null);
    
    this.addSystemMessage(room, `🔍 ${player.username} used the INTERROGATION Power-Up!`);
    opponents.forEach(opp => {
       const isEven = opp.secretNumber! % 2 === 0;
       this.addSystemMessage(room, `↳ ${opp.username}'s secret number is ${isEven ? 'EVEN' : 'ODD'}.`);
    });
  ...
  public submitGuess(
    ...
    const hints: { [playerId: string]: 'UP' | 'DOWN' | 'CORRECT' } = {};
    const hintDisplayMap: { [playerName: string]: 'UP' | 'DOWN' | 'CORRECT' } = {};
    const eliminatedThisTurn: Player[] = [];
  ...
  private nextTurn(room: Room, onUpdate: (room: Room) => void) {
    const activePlayers = room.players.filter((p) => !p.isEliminated);
    if (activePlayers.length <= 1) {
      this.checkWinConditions(room, onUpdate);
      return;
  ...
  private checkWinConditions(room: Room, onUpdate: (room: Room) => void) {
    const activePlayers = room.players.filter((p) => !p.isEliminated);
    if (activePlayers.length === 1) {
  ...
  private endRound(room: Room, winner: Player, onUpdate: (room: Room) => void) {
    this.clearTimer(room.id);
    room.status = 'ended';
  ...
  public addChatMessage(roomId: string, senderId: string, text: string): ChatMessage | undefined {
    const room = this.getRoom(roomId);
    if (!room || !room.settings.chatEnabled) return undefined;
  ...
  private addSystemMessage(room: Room, text: string) {
  ...
  private startRoomExpiryTimer(roomId: string) {
    this.clearRoomExpiryTimer(roomId);
  ...
  private clearRoomExpiryTimer(roomId: string) {
